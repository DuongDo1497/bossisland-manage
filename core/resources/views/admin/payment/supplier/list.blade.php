            ...
            <div class="card mb-3">
                <div class="card-body d-flex justify-content-between flex-wrap gap-3 align-items-center">
                    <div class="d-flex gap-3">
                        ...
                        <div class="d-flex flex-column flex-wrap">
                            <span class="text-end">
                                {{ showAmount($supplier->totalReceivableAmount()) }} {{ __($general->cur_text) }}
                            </span>

                            <span class="text-end">
                                {{ showAmount($supplier->totalPayableAmount()) }} {{ __($general->cur_text) }}
                            </span>
                        </div>
                    </div>
                        ...
                        <table class="table table--light style--two custom-data-table">
                            ...
                            <tbody>
                                ...
                                @forelse($purchases as $purchase)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($purchase->due_amount < 0)
                                                <span class="text--green">{{ showAmount(abs($purchase->due_amount)) }}
                                                    {{ $general->cur_text }}</span>
                                            @else
                                                <span class="text--danger"> {{ showAmount(abs($purchase->due_amount)) }}
                                                    {{ $general->cur_text }} </span>
                                            @endif
                                        </td>
                                    </tr>
                                    ...
                                    @if ($purchase->purchaseReturn)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($purchase->purchaseReturn->due_amount < 0)
                                                    <span class="text--danger">{{ showAmount(abs($purchase->purchaseReturn->due_amount)) }}
                                                        {{ $general->cur_text }}</span>
                                                @else
                                                    <span class="text--green">
                                                        {{ showAmount(abs($purchase->purchaseReturn->due_amount)) }}
                                                        {{ $general->cur_text }} </span>
                                                @endif
                                            </td>
                                        </tr>
                            ...
                            @if ($totalPayable < 0)
                                <tr>
                                    ...
                                    <td class="fw-bold">
                                        <span class="text--green">{{ showAmount(abs($totalPayable)) }} {{ __($general->cur_text) }}</span>
                                    </td>
                                </tr>
                            @else
                                <tr>
                                    ...
                                    <td class="fw-bold">
                                        <span class="text--danger">{{ showAmount(abs($totalPayable)) }} {{ __($general->cur_text) }}</span>
                                    </td>
                                </tr>
                            @endif
                        </table><!-- table end -->
                    ...
                    <div class="modal-body">
                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item d-flex justify-content-between flex-wrap fw-bold">
                                ...
                                <span>
                                    {{ showAmount(abs($totalPayable)) }} {{ __($general->cur_text) }}
                                </span>
                            </li>
...
@push('breadcrumb-plugins')
    <x-back route="{{ route('admin.supplier.index') }}"></x-back>
@endpush
